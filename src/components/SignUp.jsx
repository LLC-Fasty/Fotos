  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      const {
        ...
        Result,
        error: rerror,
      } = await connectRustFun(username, password, operation);
      if (rerror) {
        handleError(rerror);
      }
      if (rsuccess) {
      ...
      const { success, userDetails, error } = await createAccount(userData);
      if (error) {
        handleError(error);
      }
      if (success) {
    ...
    <div className="flex justify-center items-center min-h-screen">
      <div className="w-full md:w-[600px]">
        ...
        >
          ...
          <h2 className="text-[#0083ff] md:text-2xl text-xl font-black mt-4 text-center">
